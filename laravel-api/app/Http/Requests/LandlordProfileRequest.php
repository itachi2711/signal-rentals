...
class LandlordProfileRequest extends BaseRequest
{
            ...
            case 'PATCH':
            {
                $rules = [
                    ...
                    'phone' => ['nullable', 'regex:/^([0-9\s\-\+\(\)]*)$/',
                        Rule::unique('landlords')->ignore($this->landlord_profile, 'id')
                            ->where(function ($query) {
                                $query->where('deleted_at', NULL);
                            })],
                    'email' => ['required', Rule::unique('landlords')->ignore($this->landlord_profile, 'id')
                        ->where(function ($query) {
                            $query->where('deleted_at', NULL);
                        })],
